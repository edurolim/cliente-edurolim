...
def load_prompts():
    ...
    if len(prompts) != 8:
        raise RuntimeError(f"Expected 8 prompts, found {len(prompts)}")
    return prompts


def request_image(api_key, prompt):
    payload = {
        "model": "gpt-image-1",
        "prompt": prompt,
        "size": "1024x1536",
...
def main():
    ...
    for index, prompt in prompts:
        out_path = IMG_DIR / f"slide_{index:02d}.png"
        ...
        print(f"slide {index:02d}: saved {out_path}", flush=True)
        time.sleep(1)
